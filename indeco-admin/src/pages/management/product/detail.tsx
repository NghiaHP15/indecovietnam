...
export const ProductDetailForm = forwardRef(
  ({ readOnly, setLoading, reload, closeModal }: FormProps, ref) => {
    ...
    const onEditorChange = (value: string, field: PropKey) => {
      if(readOnly) return;
      if(field === "body") setContent(value);
      if(field === "policy") setPolicy(value);
    }
    ...
    return (
      ...
      <Form layout="vertical">
        <Row gutter={24}>
          ...
          <Col span={24}>
            <Form.Item
              label={t('website.product.field.policy')}
            >
              <Editor 
                sample={false} value={policy || ""} 
                onChange={(e) => onEditorChange(e, "policy")}
                className=''
              />
            </Form.Item>
          </Col>
